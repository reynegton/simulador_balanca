...
class GVariantTextCodec {
  ...
  DBusValue decode(String type, String data) {
    final buffer = _DecodeBuffer(data);
    final value = _decode(type, buffer);
  ...
  void _encode(StringBuffer buffer, DBusValue value) {
    if (value is DBusBoolean) {
      ...
      if (value.value != null) {
        ...
        final childText = childBuffer.toString();
      ...
      switch (rune) {
        ...
        default:
          ...
          if (rune <= 0x1f ||
            ...
            final hex = rune.toRadixString(16).padLeft(padding, '0');
    ...
    switch (type) {
      case 'b': // boolean
        ...
        if (buffer.consume('true')) {
          ...
          throw const FormatException('Invalid boolean encoding');
        }
      ...
      case 'o': // object path
        if (!buffer.consume('objectpath ')) {
          throw const FormatException('Invalid object path encoding');
        }
      ...
      default:
        throw ArgumentError.value(
          type,
          'type',
          "Unsupported GVariant type: '$type'",
        );
    }
  }
  ...
  int _decodeInteger(_DecodeBuffer buffer) {
    ...
    final value = int.parse(buffer.data.substring(buffer.offset, end));
    buffer.offset = end;
    return value;
  }
  ...
  double _decodeDouble(_DecodeBuffer buffer) {
    ...
    final value = double.parse(buffer.data.substring(buffer.offset, end));
    buffer.offset = end;
    return value;
  }
  ...
  String _decodeString(_DecodeBuffer buffer) {
    ...
    if (buffer.isEmpty) {
      throw const FormatException('No data for string');
    }
    final quote = buffer.data[buffer.offset];
    if (quote != "'" && quote != '"') {
      throw const FormatException('Missing start quote on string');
    }
    var end = buffer.offset + 1;
    while (end < buffer.data.length) {
      final c = buffer.data[end];
      end++;
      if (c == quote) {
        buffer.offset = end;
        ...
        if (end == buffer.data.length - 1) {
          throw const FormatException('Escape character at end of string');
        }
        final escapeChar = buffer.data[end];
        end++;
        switch (escapeChar) {
          case 'a': // bell
          ...
          case 'u':
            if (end + 4 > buffer.data.length) {
              throw const FormatException(
                'Not enough space for unicode character',
              );
            }
    ...
    throw const FormatException('Missing end quote on string');
  }

  DBusStruct _decodeStruct(String type, _DecodeBuffer buffer) {
    if (!buffer.consume('(')) {
      throw const FormatException('Missing start of struct');
    }
    final signature = DBusSignature(type.substring(1, type.length - 1));
    final children = <DBusValue>[];
    var first = true;
    for (final childSignature in signature.split()) {
      if (!first && !buffer.consume(',')) {
        throw const FormatException('Missing comma between struct elements');
      }
      first = false;
      buffer.consumeWhitespace();
    ...
    if (!buffer.consume(')')) {
      throw const FormatException('Missing end of struct');
    }
  ...
  DBusArray _decodeArray(String type, _DecodeBuffer buffer) {
    if (!buffer.consume('[')) {
      throw const FormatException('Missing start of array');
    }
    final childType = type.substring(1);
    final children = <DBusValue>[];
    var first = true;
    while (!buffer.isEmpty) {
      ...
      if (!first && !buffer.consume(',')) {
        throw const FormatException('Missing comma between array elements');
      }
      first = false;
      buffer.consumeWhitespace();
      children.add(_decode(childType, buffer));
    }
    throw const FormatException('Missing end of array');
  }

  DBusDict _decodeDict(String type, _DecodeBuffer buffer) {
    final signatures =
        DBusSignature(type.substring(2, type.length - 1)).split();
    final keyType = signatures[0].value;
    final valueType = signatures[1].value;
    if (!buffer.consume('{')) {
      throw const FormatException('Missing start of dict');
    }
    final children = <DBusValue, DBusValue>{};
    var first = true;
    while (!buffer.isEmpty) {
      if (buffer.consume('}')) {
        return DBusDict(
          DBusSignature(keyType),
          DBusSignature(valueType),
          children,
        );
      }

      if (!first && !buffer.consume(',')) {
        throw const FormatException('Missing comma between dict elements');
      }
      first = false;
      buffer.consumeWhitespace();

      final key = _decode(keyType, buffer);
      buffer.consumeWhitespace();
      if (!first && !buffer.consume(':')) {
        throw const FormatException('Missing colon between dict key and value');
      }
      buffer.consumeWhitespace();
      final value = _decode(valueType, buffer);

      children[key] = value;
    }
    throw const FormatException('Missing end of dict');
  }
}
